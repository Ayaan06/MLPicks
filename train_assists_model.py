"""Train the assists regression model."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train assists projection model.")
    parser.add_argument(
        "--data-path",
        type=Path,
        default=Path("data/player_games_assists.csv"),
...
def main() -> None:
    ...
    model, metrics = model_store.train_regression_model(
        ...
        target_col="assists_actual",
    )
    sigma = metrics["residual_std"]
    model_store.save_model(
        model=model,
        sigma=sigma,
        model_path=model_store.ASSISTS_MODEL_PATH,
        sigma_path=model_store.ASSISTS_SIGMA_PATH,
    )
...
def _persist_feature_importance(model, feature_cols: list[str]) -> None:
    ...
    model_store.save_feature_importance_scores("assists", scores)
...
def _update_metadata(version: str, df: pd.DataFrame, metrics: dict) -> None:
    ...
    model_store.update_metadata_entry("assists_model", payload)
